...
class Texture:
    """create moderngl texture objects for rendering"""
    ...
    def get_texture_cube(self, dir_path, ext='png'):
        ...
        texture_cube = self.ctx.texture_cube(size=size, components=4, data=None)

        for i in range(6):
            texture_data = pg.image.tostring(textures[i], 'RGBA')
            texture_cube.write(face=i, data=texture_data)

        return texture_cube

    def get_texture(self, path):
        """allocate simple texture"""
